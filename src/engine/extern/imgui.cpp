    ...
    for (size_t i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseDown[i])
            if (mouse_earliest_button_down == -1 || g.IO.MouseClickedTime[mouse_earliest_button_down] > g.IO.MouseClickedTime[i])
                mouse_earliest_button_down = static_cast<int>(i);
    }
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (flags & ImGuiInputTextFlags_CallbackCharFilter)
    {
        ...
        callback_data.EventChar = static_cast<ImWchar>(c);
    ...
    if (focus_requested || user_clicked)
    {
        if (g.ActiveId != id)
        {
            ...
            else
            {
                ...
                edit_state.StbState.cursor = ImMin(edit_state.StbState.cursor, static_cast<int>(edit_state.CurLenW));
                edit_state.StbState.select_start = ImMin(edit_state.StbState.select_start, static_cast<int>(edit_state.CurLenW));
                edit_state.StbState.select_end = ImMin(edit_state.StbState.select_end, static_cast<int>(edit_state.CurLenW));
            }
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (is_ctrl_down && (IsKeyPressedMap(ImGuiKey_X) || IsKeyPressedMap(ImGuiKey_C)))
        {
            ...
            if (g.IO.SetClipboardTextFn)
            {
                ...
                const int ie = edit_state.HasSelection() ? ImMax(edit_state.StbState.select_start, edit_state.StbState.select_end) : static_cast<int>(edit_state.CurLenW);
